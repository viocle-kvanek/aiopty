//go:build windows && go1.11 && !go1.21
// +build windows,go1.11,!go1.21

// See exec_go121.go to get more information.
...
func (p *ConPty) startProcessWithConPty(argv0 string, argv []string, attr *syscall.ProcAttr, pseudoConsole *syscall.Handle) (pid int, handle uintptr, err error) {
	...
	if sys.Token != 0 {
		err = syscall.CreateProcessAsUser(sys.Token, argv0p, argvp, nil, nil, true, flags, envBlock, dirp, &si.StartupInfo, pi)
	} else {
